<template>
  <div class="card">
    <h3 class="text-lg font-bold mb-4" style="font-family: var(--font-heading); color: var(--text-primary)">
      {{ isEditing ? "编辑文章" : "新建文章" }}
    </h3>
    <form @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2">
          <button type="button" @click="$emit('cancel')" class="px-4 py-2 rounded-lg text-sm cursor-pointer" style="background: var(--bg-base); color: var(--text-secondary); border: 1px solid var(--border)">取消</button>
...
const props = defineProps<{
  post?: { id: string; slug: string; title: string; summary: string; content: string; category: string; tags: string[]; published: boolean }
}>()
