...
| 用途 | 字体 | 来源 | 说明 |
|---|---|---|---|
| **品牌名/Logo** | 装饰性中文 Display 字体 | 待选型（见下方） | 粗壮笔画，类似"站酷快乐体"风格，笔画有设计感，是视觉焦点 |
...
### 2.2 品牌名 Display 字体选型候选

用户要求"少吃熏鱼"使用类似参考图中"橘猫小窝"风格的装饰性中文字体。以下为候选字体（需进一步确认最终选择）：

1. **站酷快乐体** — 免费商用，笔画圆润活泼，辨识度高
2. **站酷高端黑** — 免费商用，笔画刚硬有力，更偏赛博朋克气质
3. **阿里巴巴普惠体（粗黑）** — 免费商用，笔画厚重，现代感强
4. **得意黑（Smiley Sans）** — 开源，笔画有手写感但结构清晰，独特但不过度
5. **LXGW WenKai（霞鹜文楷）** — 开源，楷书风格但数字化处理，适合需要"文化感"的场景

> **待办：** 用户从上方候选字体中选定最终品牌字体，或提供其他字体。

### 2.3 字体加载策略

- 使用 Google Fonts CDN 或字蛛（font-spider）按需加载
...
:root {
  ...
  /* 背景层级 */
  --bg-deep: #0b0b14;              /* 最底层深空背景 */
  --bg-surface: rgba(14, 14, 28, 0.6);  /* 玻璃面板底色 */

  /* 液态玻璃 */
  --glass: rgba(255, 255, 255, 0.035);        /* 面板填充 */
  --glass-thick: rgba(255, 255, 255, 0.06);    /* 加重填充（hover） */
  --glass-border: rgba(255, 255, 255, 0.08);    /* 面板边框 */
  --glass-border-hover: rgba(255, 255, 255, 0.15);  /* hover 边框 */
  --glass-blur: 24px;                           /* 毛玻璃模糊强度 */
  ...
  /* 文字 */
  --text-primary: #f5f3ff;          /* 主要文字（近白） */
  --text-body: #c8c4d8;             /* 正文（柔和灰紫） */
  --text-secondary: #7e7a92;        /* 辅助文字（暗灰紫） */
...
使用多个固定定位的 radial-gradient 元素（`<div>` 或伪元素），颜色为霓虹色的极低透明度版本，缓慢 CSS 动画移动位置。
...
- 使用选定的**装饰性中文 Display 字体**渲染
- 默认状态下为 `--text-primary`（暗色模式下近白），可通过 CSS 变量覆盖
...
- 默认：单色（`--text-primary` 或自定义色）
- 可选：渐变填充 `background: var(--gradient-brand)` + `-webkit-background-clip: text`
- 不使用描边、阴影或外发光，让字体本身的设计感说话
...
- [ ] 确认品牌名 Display 字体最终选择
- [ ] 确认明暗模式切换按钮的具体样式
- [ ] 确认首页 Hero 区是否需要额外视觉元素（如 ASCII art、代码片段装饰）
- [ ] 确认卡片 hover 效果的精确参数（上移距离、过渡时间）
